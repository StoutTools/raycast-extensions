  ...
  useEffect(() => {
    ...
    if (isPolling) {
      intervalId = setInterval(async () => {
        // Get current recentScans from state to avoid stale closure
        const currentScans = await LocalStorage.getItem<string>('recentScans');
        if (!currentScans) {
          setIsPolling(false);
          return;
        }

        const parsedScans = JSON.parse(currentScans) as RecentScanItem[];
        const activeScans = parsedScans.filter((scan) => scan.status === 'PENDING' || scan.status === 'RUNNING');
        ...
        for (const scan of activeScans) {
          try {
            ...
            console.error(`Failed to poll scan ${scan.id}:`, error);
          }
        }
