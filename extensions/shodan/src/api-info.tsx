...
import { ShodanAPI } from './shodan-api';
import {
  getUsageInfo,
...
export default function APIInfo() {
  ...
  const {
    ...
    async () => {
      return await shodanAPI.getAPIInfo();
    },
    [],
    {
      onError: (error) => {
  ...
  if (error) {
    return (
      <Detail
        markdown={generateErrorMarkdown(error.message)}
        actions={
          <ActionPanel>
  ...
  return (
    <List
      ...
      actions={
        <ActionPanel>
          <Action title="Refresh Data" icon={Icon.ArrowClockwise} onAction={revalidate} />
